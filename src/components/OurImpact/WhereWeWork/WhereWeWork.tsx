...
export default function WhereWeWork() {
  ...
  const beforeInjection = (svg: SVGElement) => {
    // This part removing text is fine
    const textElements = svg.querySelectorAll('text');
    textElements.forEach(text => {
      // ... your text removal logic
    });

    const bhaktapurPoly = document.createElementNS("http://www.w3.org/2000/svg", "polygon");
    bhaktapurPoly.setAttribute("points", bhaktapurPolygon);
    
    // --- FIX #1: Set the correct initial state ---
    // Make the polygon invisible by default so it only appears on hover.
    bhaktapurPoly.setAttribute("fill", 'transparent');
    bhaktapurPoly.setAttribute("stroke", 'none');
    
    bhaktapurPoly.setAttribute("stroke-width", "2");
    bhaktapurPoly.style.cursor = 'pointer';
    bhaktapurPoly.style.transition = 'all 0.2s ease';
    // pointer-events must be 'auto' or 'all' for the browser to detect hovering on a transparent element.
    bhaktapurPoly.style.pointerEvents = 'auto'; 
    
    bhaktapurPoly.addEventListener('mouseenter', (e) => {
      // Set the desired hover colors
      bhaktapurPoly.setAttribute("fill", '#F1AA19'); // Cyan on hover
      bhaktapurPoly.setAttribute("stroke", '#00B6B8'); // Darker cyan stroke
    ...
    bhaktapurPoly.addEventListener('mouseleave', () => {
      // --- FIX #2: Reset the state correctly ---
      // Revert to the invisible state when the mouse leaves.
      bhaktapurPoly.setAttribute("fill", 'transparent');
      bhaktapurPoly.setAttribute("stroke", 'none');
      
      // Hide the popup. This will now work as expected.
      setPopupData(null);
    });
        ...
        {popupData && (
          <div 
            ...
            style={{ 
              ...
              top: popupPosition.y,
              // Offset the popup to appear above the cursor
